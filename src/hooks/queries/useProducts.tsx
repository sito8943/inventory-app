    ...
    return useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const cached = await loadCache(Tables.Products) as CategoryProductDto[];
    ...
    return useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                console.warn("API failed, loading categories from cache", error);
                const cached = await loadCache(Tables.MovementLogs) as MovementLogDto[];
                if (!cached || !Array.isArray(cached)) throw new Error("No cached categories available");
                return {items: cached.filter((movementLog) => movementLog.product === id), total: cached?.length};
            }
        },
    });
};
