...
// types
import { RowCellPropsType } from "./components/types.ts";
import { UseTablePropsType, TablePropsType, TableColumn } from "./types.ts";

export const useTableOptions = <TRowDto extends {}>(
  props: UseTablePropsType<TRowDto>,
): TablePropsType<TRowDto> => {
  ...
  const rows = useMemo((): TRowDto[] => {
    if (!data) return [];

    return data.map((item: any) => {
      const row = { ...item };
      ignoreColumns.forEach((column: string) =>
        column !== "id" ? delete row[column] : null,
      );

      Object.keys(row).forEach((column) => {
        const newRow: RowCellPropsType = { value: undefined };
